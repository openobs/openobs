...
-- `CREATE INDEX IF NOT EXISTS` so the call is idempotent.
--
-- There is no migration runner: this is a fresh-install schema. Schema edits
-- happen by editing this file directly. If a deployed instance ever needs to
-- evolve, that's a future concern handled out of band (rebuild + re-seed, or
-- a one-off ALTER script run by the operator).
...
-- ============================================================================
-- LLM call audit log (Task 04)
--
-- One row per LLM gateway invocation. Privacy-critical: never store raw
-- prompt text, user messages, tool outputs, API keys, or error message
-- bodies — only the sha256 prompt hash, token counts, latency, cost, and
-- enum-bucketed error kind. See packages/llm-gateway/src/audit.ts.
-- ============================================================================
