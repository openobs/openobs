/**
 * DatasourceRepository — CRUD for `instance_datasources`.
 *
 * Secret columns (api_key, password) are encrypted on write via
 * AES-256-GCM with SECRET_KEY and decrypted on read. Callers pass
 * `{ masked: true }` to receive redacted values for UI.
 */
...
export class DatasourceRepository implements IDatasourceRepository {
  ...
  async list(opts: ListDatasourcesOptions): Promise<InstanceDatasource[]> {
    ...
    const whereClause = wheres.length
      ...
      : sql``;
    const rows = this.db.all<DatasourceRow>(sql`
