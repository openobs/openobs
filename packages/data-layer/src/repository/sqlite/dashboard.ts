/**
 * DashboardRepository — SQLite-backed dashboard store (W6 / T6.A1).
 *
 ...
 *   - No capacity eviction — SQLite is the durable store and we rely on
 ...
 *
 * Per ADR-001 M1: this is the canonical SQLite implementation of
 * `IDashboardRepository`. The legacy `SqliteDashboardRepository` wrapper
 * was removed when the data-layer `RepositoryBundle.dashboards` was
 * retyped to the canonical interface.
 */
...
export class DashboardRepository implements IDashboardRepository {
  ...
  async findById(id: string): Promise<Dashboard | null> {
    const rows = this.db.all<DashboardRow>(
  ...
  async findAll(userId?: string): Promise<Dashboard[]> {
    const rows =
      userId === undefined
        ? this.db.all<DashboardRow>(sql`SELECT * FROM dashboards ORDER BY created_at DESC`)
        : this.db.all<DashboardRow>(
  ...
  async delete(id: string): Promise<boolean> {
    ...
    this.db.run(sql`DELETE FROM dashboards WHERE id = ${id}`);
    return true;
  }

  async size(): Promise<number> {
    const rows = this.db.all<{ n: number }>(sql`SELECT COUNT(*) AS n FROM dashboards`);
    return rows[0]?.n ?? 0;
  }

  async clear(): Promise<void> {
    this.db.run(sql`DELETE FROM dashboards`);
  }
