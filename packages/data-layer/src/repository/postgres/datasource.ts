 ...
 *
 * Note on `is_default`: both backends store INTEGER 0/1 in the runtime DDL.
 * `toBool` from instance-shared still accepts booleans for older rows/tests.
 */
...
export class PostgresDatasourceRepository implements IDatasourceRepository {
  ...
  async list(opts: ListDatasourcesOptions): Promise<InstanceDatasource[]> {
    ...
    const whereClause = wheres.length
      ...
      : sql``;
    const rows = await this.db.all<DatasourceRow>(sql`
  ...
  async get(id: string, opts: DatasourceLookupOptions): Promise<InstanceDatasource | null> {
    const rows = await this.db.all<DatasourceRow>(
  ...
  async delete(id: string, orgId: string): Promise<boolean> {
    ...
    await this.db.run(sql`DELETE FROM instance_datasources WHERE id = ${id} AND org_id = ${orgId}`);
    return true;
  }

  async count(orgId: string): Promise<number> {
    const rows = await this.db.all<{ n: number | string }>(
