...
export class ApiKeyRepository implements IApiKeyRepository {
  ...
  async findById(id: string): Promise<ApiKey | null> {
    const rows = await pgAll<Row>(this.db, sql`SELECT * FROM api_key WHERE id = ${id}`);
  ...
  async delete(id: string): Promise<boolean> {
    ...
    await pgRun(this.db, sql`DELETE FROM api_key WHERE id = ${id}`);
    return true;
  }

  async touchLastUsed(id: string, at: string): Promise<void> {
    await pgRun(this.db, sql`UPDATE api_key SET last_used_at = ${at} WHERE id = ${id}`);
  }
}
