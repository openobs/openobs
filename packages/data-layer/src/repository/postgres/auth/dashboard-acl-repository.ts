...
export class DashboardAclRepository implements IDashboardAclRepository {
  ...
  async findById(id: string): Promise<DashboardAcl | null> {
    const rows = await pgAll<Row>(this.db, sql`SELECT * FROM dashboard_acl WHERE id = ${id}`);
  ...
  async delete(id: string): Promise<boolean> {
    ...
    await pgRun(this.db, sql`DELETE FROM dashboard_acl WHERE id = ${id}`);
    return true;
  }
  ...
  async deleteByDashboard(dashboardId: string): Promise<number> {
    ...
    await pgRun(this.db, sql`DELETE FROM dashboard_acl WHERE dashboard_id = ${dashboardId}`);
    return before[0]?.n ?? 0;
  }
  ...
  async deleteByFolder(folderId: string): Promise<number> {
    ...
    await pgRun(this.db, sql`DELETE FROM dashboard_acl WHERE folder_id = ${folderId}`);
    return before[0]?.n ?? 0;
  }
}
