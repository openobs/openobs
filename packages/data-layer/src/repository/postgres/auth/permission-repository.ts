...
export class PermissionRepository implements IPermissionRepository {
  ...
  async findById(id: string): Promise<Permission | null> {
    const rows = await pgAll<Row>(this.db, sql`SELECT * FROM permission WHERE id = ${id}`);
  ...
  async delete(id: string): Promise<boolean> {
    ...
    await pgRun(this.db, sql`DELETE FROM permission WHERE id = ${id}`);
    return true;
  }
  ...
  async deleteByRole(roleId: string): Promise<number> {
    ...
    await pgRun(this.db, sql`DELETE FROM permission WHERE role_id = ${roleId}`);
    return before[0]?.n ?? 0;
  }
}
