...
export class PreferencesRepository implements IPreferencesRepository {
  ...
  async upsert(input: NewPreferences): Promise<Preferences> {
    ...
    const existingRows = await pgAll<Row>(this.db, findKeyQuery(input.orgId, userId, teamId));
    ...
    if (existing) {
      ...
      `);
      const updated = await pgAll<Row>(this.db, sql`SELECT * FROM preferences WHERE id = ${existing.id}`);
    ...
    `);
    const rows = await pgAll<Row>(this.db, sql`SELECT * FROM preferences WHERE id = ${id}`);
  ...
  async findOrgPrefs(orgId: string): Promise<Preferences | null> {
    const rows = await pgAll<Row>(this.db, findKeyQuery(orgId, null, null));
    return rows[0] ? rowTo(rows[0]) : null;
  }

  async findUserPrefs(orgId: string, userId: string): Promise<Preferences | null> {
    const rows = await pgAll<Row>(this.db, findKeyQuery(orgId, userId, null));
    return rows[0] ? rowTo(rows[0]) : null;
  }

  async findTeamPrefs(orgId: string, teamId: string): Promise<Preferences | null> {
    const rows = await pgAll<Row>(this.db, findKeyQuery(orgId, null, teamId));
    return rows[0] ? rowTo(rows[0]) : null;
  }

  async update(id: string, patch: PreferencesPatch): Promise<Preferences | null> {
    const existingRows = await pgAll<Row>(this.db, sql`SELECT * FROM preferences WHERE id = ${id}`);
    const existing = existingRows[0];
  ...
  async delete(id: string): Promise<boolean> {
    const before = await pgAll<Row>(this.db, sql`SELECT id FROM preferences WHERE id = ${id}`);
    if (before.length === 0) return false;
    await pgRun(this.db, sql`DELETE FROM preferences WHERE id = ${id}`);
    return true;
  }
}
