...
export class OrgRepository implements IOrgRepository {
  ...
  async findById(id: string): Promise<Org | null> {
    const rows = await pgAll<OrgRow>(this.db, sql`SELECT * FROM org WHERE id = ${id}`);
    return rows[0] ? rowToOrg(rows[0]) : null;
  }

  async findByName(name: string): Promise<Org | null> {
    const rows = await pgAll<OrgRow>(this.db, sql`SELECT * FROM org WHERE name = ${name}`);
    return rows[0] ? rowToOrg(rows[0]) : null;
  }
  ...
  async list(opts: ListOptions = {}): Promise<Page<Org>> {
    ...
    const rows = await pgAll<OrgRow>(this.db,
      sql`SELECT * FROM org ORDER BY name LIMIT ${limit} OFFSET ${offset}`,
    );
    const totalRow = await pgAll<{ n: number }>(this.db, sql`SELECT COUNT(*) AS n FROM org`);
  ...
  async listWithUserCounts(
    ...
    const rows = await pgAll<OrgRow & { user_count: number }>(this.db, sql`
  ...
  async delete(id: string): Promise<boolean> {
    ...
    await pgRun(this.db, sql`DELETE FROM org WHERE id = ${id}`);
    return true;
  }
}
