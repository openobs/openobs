...
export class UserAuthRepository implements IUserAuthRepository {
  ...
  async findById(id: string): Promise<UserAuth | null> {
    const rows = await pgAll<UserAuthRow>(this.db, sql`SELECT * FROM user_auth WHERE id = ${id}`);
    return rows[0] ? rowTo(rows[0]) : null;
  }

  async findByAuthInfo(authModule: string, authId: string): Promise<UserAuth | null> {
    const rows = await pgAll<UserAuthRow>(this.db, sql`
  ...
  async listByUser(userId: string): Promise<UserAuth[]> {
    const rows = await pgAll<UserAuthRow>(this.db,
  ...
  async delete(id: string): Promise<boolean> {
    ...
    await pgRun(this.db, sql`DELETE FROM user_auth WHERE id = ${id}`);
    return true;
  }
  ...
  async deleteByUser(userId: string): Promise<number> {
    ...
    await pgRun(this.db, sql`DELETE FROM user_auth WHERE user_id = ${userId}`);
    return n;
  }
}
