...
export class PostgresFeedItemRepository implements IFeedItemRepository {
  ...
  async getStats(): Promise<FeedbackStats> {
    ...
    const withFeedback = items.filter((i: any) => i.feedback).length;
    ...
    for (const item of items) {
      if (item.feedback) {
        byVerdict[item.feedback as FeedFeedback] = (byVerdict[item.feedback as FeedFeedback] ?? 0) + 1;
      }
    }
    ...
    const proactiveItems = items.filter((i: any) => proactiveTypes.includes(i.type as FeedEventType));
    const proactiveHitRate = proactiveItems.length > 0
      ? proactiveItems.filter((i: any) => i.followed_up === true).length / proactiveItems.length
      : 0;
