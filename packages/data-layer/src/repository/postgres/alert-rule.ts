...
export class AlertRuleRepository implements IAlertRuleRepository {
  ...
  async findById(id: string): Promise<AlertRule | undefined> {
    const rows = await pgAll<RuleRow>(this.db, sql`SELECT * FROM alert_rules WHERE id = ${id}`);
  ...
  async findAll(filter: AlertRuleFindAllOptions = {}): Promise<{ list: AlertRule[]; total: number }> {
    ...
    let rows = await pgAll<RuleRow>(this.db, sql`
  ...
  async findByWorkspace(workspaceId: string): Promise<AlertRule[]> {
    const rows = await pgAll<RuleRow>(this.db,
  ...
  async delete(id: string): Promise<boolean> {
    ...
    await pgRun(this.db, sql`DELETE FROM alert_rules WHERE id = ${id}`);
    return true;
  }
  ...
  async getHistory(ruleId: string, limit = 50): Promise<AlertHistoryEntry[]> {
    const rows = await pgAll<HistoryRow>(this.db, sql`
  ...
  async createSilence(data: Omit<AlertSilence, 'id' | 'createdAt'>): Promise<AlertSilence> {
    ...
    `);
    const rows = await pgAll<SilenceRow>(this.db, sql`SELECT * FROM alert_silences WHERE id = ${id}`);
    if (!rows[0])
  ...
  async findSilences(): Promise<AlertSilence[]> {
    const now = nowIso();
    const rows = await pgAll<SilenceRow>(this.db,
  ...
  async findAllSilencesIncludingExpired(): Promise<AlertSilence[]> {
    const rows = await pgAll<SilenceRow>(this.db, sql`SELECT * FROM alert_silences`);
    return rows.map(rowToSilence);
  }
  ...
  async updateSilence(
    ...
    await pgRun(this.db, sql`
      ...
      WHERE id = ${id}
    `);
    const updated = await pgAll<SilenceRow>(this.db, sql`SELECT * FROM alert_silences WHERE id = ${id}`);
    return updated[0] ? rowToSilence(updated[0]) : undefined;
  }

  async deleteSilence(id: string): Promise<boolean> {
    const rows = await pgAll<SilenceRow>(this.db, sql`SELECT id FROM alert_silences WHERE id = ${id}`);
    if (rows.length === 0) return false;
    await pgRun(this.db, sql`DELETE FROM alert_silences WHERE id = ${id}`);
    return true;
  }
  ...
  async findAllPolicies(): Promise<NotificationPolicy[]> {
    const rows = await pgAll<PolicyRow>(this.db, sql`SELECT * FROM notification_policies`);
    return rows.map(rowToPolicy);
  }

  async findPolicyById(id: string): Promise<NotificationPolicy | undefined> {
    const rows = await pgAll<PolicyRow>(this.db,
  ...
  async deletePolicy(id: string): Promise<boolean> {
    ...
    await pgRun(this.db, sql`DELETE FROM notification_policies WHERE id = ${id}`);
    return true;
  }
}
 ...
 * `PostgresAlertRuleRepository`. Keep the name exported so the parent's
 * factory reconciliation in a follow-up commit doesn't crash the build
 * before it lands.
 */
export const PostgresAlertRuleRepository = AlertRuleRepository;
export type PostgresAlertRuleRepository = AlertRuleRepository;
