...
export async function runProactivePipeline(): Promise<void> {
  if (started)
    return;
  started = true;
  ...
  queue.process<{ serviceId: string; metrics: Record<string, number> }>(
    'anomaly-check',
    async (job) => {
  ...
  queue.process<{ symptoms?: unknown[]; changes?: unknown[] }>(
    'correlate',
    async (job) => {
  ...
  let alertRuleEvaluator: AlertRuleEvaluator | undefined;
  if (prometheusUrl) {
  ...
  const pipeline = createProactivePipeline(
    { correlationEngine, alertRuleEvaluator },
    { feed: feedStore, incidents: incidentStore },
  );
