...
export class DashboardGeneratorAgent {
  ...
  private async selectRelevantMetrics(goal: string, allMetrics: string[]): Promise<string[]> {
...
## Output Format (JSON)
{
  ...
  private async generateAndCriticLoop(
    group: PanelGroup,
    input: GenerateInput,
...
approved = true if overallScore >= 8 AND no severity=error issues.`
  ...
  private async validateQueries(panels: PanelConfig[]): Promise<PanelConfig[]> {
    if (!this.deps.prometheusUrl)
      return panels
