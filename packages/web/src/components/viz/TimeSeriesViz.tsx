/**
 * TimeSeriesViz — a Grafana-quality time-series panel for Rounds.
 *
...
export interface TimeSeriesVizProps {
   ...
   * receive each other's cursor position. Defaults to `'rounds-panels'` so
...
export function TimeSeriesViz(props: TimeSeriesVizProps): JSX.Element {
  ...
  useEffect(() => {
    return subscribeCursor(syncKey ?? 'rounds-panels', sourceId, (detail) => {
    ...
    const setCursor = (u: uPlot): void => {
      ...
      if (idx === null || idx === undefined || idx < 0 || cLeft < 0) {
        ...
        if (!applyingExternalRef.current && isLocal) {
          publishCursor({ ts: null, sourceId, syncKey: syncKey ?? 'rounds-panels' });
        }
        return;
      }
