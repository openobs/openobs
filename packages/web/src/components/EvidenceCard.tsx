...
const TYPE_META: Record<Evidence['type'], { icon: string; color: string; label: string }> = {
  ...
  trace: { icon: '|', color: 'text-[var(--color-on-surface-variant)] bg-[var(--color-surface-high)] border-[var(--color-outline-variant)]', label: 'Trace' },
...
export default function EvidenceCard({ evidence }: Props) {
  ...
  return (
    <div className="bg-[var(--color-surface-highest)] rounded-lg border border-[var(--color-outline-variant)] p-4 space-y-3">
      <div className="flex items-start justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          {evidence.reproducible ? (
            ...
            <span className="text-xs text-[var(--color-on-surface-variant)]">Not reproducible</span>
          )}
        </div>
        <span className="text-xs text-[var(--color-on-surface-variant)]">{formatTs(evidence.timestamp)}</span>
      </div>

      <p className="text-sm text-[var(--color-on-surface)] leading-relaxed">{evidence.summary}</p>
